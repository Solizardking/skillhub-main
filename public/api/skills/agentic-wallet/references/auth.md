...
Use the 6-digit code from the user's email to complete authentication. The flow ID from step 1 is saved automatically to a local file — you normally do not pass it as an argument. If you have the ability to access the user's email, you can read the OTP code, or you can ask your human for the code.

When you captured `flowId` from `auth login … --json` (or status UX text asks for both), you may pass it explicitly:

```bash
npx awal@2.12.1 auth verify <flowId> <otp>
# example:
npx awal auth verify <flow-id-from-login-json> 123456
```
